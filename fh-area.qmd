...
Area-level SAE starts from the direct estimates created in the previous workbook. The Fay--Herriot model lets direct estimates for each area-time cell, an *MSOA-year cell* in this example, borrow strength from predictors and spatial and temporal structures to stabilise noisy estimates.
...
First we join the complete area-time domain to the direct estimates and their sampling variances from the previous workbook.
...
```
Area-time cells with high sampling variance contain less reliable local information, so M0 pulls them more strongly toward its baseline prediction.
...
WAIC is useful, but it does not show where estimates change locally or whether those changes are substantive. For Fay-Herriot SAE models, the final aim is stabilising noisy local estimates. When predictors are added, the model updates the expected value for each area-time cell. Unstable direct estimates are then pulled more strongly toward those updated model predictions, which can improve local accuracy when the predictors are relevant.
