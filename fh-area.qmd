...
Now, we assemble the modelling data. Note that the models use two area-level random-effect structures, so we create separate `INLA` index variables:
...
M2 has the lowest WAIC and M0 is clearly the weakest model, suggesting that the temporal term and the two core predictors, population density and car access, already capture much of the area-level variability. The differences among M1-M4 are small, so we treat their predictive performance as broadly similar and use local diagnostics to inspect what each added term does to the estimates.

## Diagnostics of stepwise estimate changes
...
The **temporal effect** has a high `tau` (`6091.734`), which means the model estimates only small differences between the three survey waves. The effect is still useful to include because it lets the model account for wave-to-wave changes instead of treating survey responses from all waves as one.
...
fh_fits[["M1 +Temporal"]]$summary.hyperpar |>
  ...
  filter(str_detect(.data$parameter, "time_inla_id")) |>
...
The **spatial effect** has a lower `tau` (`102.617`) than the temporal effect, which suggests that there is more residual variation between MSOAs than between survey waves. However, its low `phi` (`0.032`) indicates that only a very small share of that remaining between-area variation follows the provided spatial weights. Most of the remaining area-level variation is unstructured, which helps explain why adding the spatial structure to the random area effects does not improve WAIC.
...
```
The maps show small local clusters of spatial smoothing. For example, in parts of southern Wales, where several neighbouring MSOAs have higher estimated daily-walking rates, estimates of nearby areas are raised.
...
The final model estimates should be reported with their associated uncertainty. In the maps below, uncertainty is shown as the width of the posterior 95% interval, which is easier to read than posterior variance because it is on the same percentage-point scale as the estimate. For transparency, the output should also retain the direct estimates, sample sizes, sampling variances, and modelling specification.
...
final_map_data <- msoa_boundaries |>
  left_join(final_model_results, by = "area_id")
final_estimate_limits <- range(final_model_results$fh_estimate, na.rm = TRUE)
final_ci_width_limits <- c(0, max(final_model_results$fh_ci_width, na.rm = TRUE))
final_wave_years <- sort(unique(final_model_results$time_id))
