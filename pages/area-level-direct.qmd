...
### Main Example: Mean Commute Distance
...
```{r}
#| echo: false
commute_direct_export <- msoa_domains |>
  left_join(commute_sample_n, by = "domain_id") |>
  left_join(dir_est_commute, by = "domain_id") |>
  mutate(n = replace_na(n, 0)) |>
  transmute(
    ...
    variance,
    direct_ci_lower = direct - 1.96 * sqrt(variance),
    direct_ci_upper = direct + 1.96 * sqrt(variance),
    direct_ci_width = ci_width,
    n
  )

write_csv(
  commute_direct_export,
  file.path("..", "data", "commute_estimates_direct.csv")
)
```
...
$$
\hat{\theta}_d^{DIR}=\frac{\sum_iw^G_{di}y_{di}}{N_d}
$$
