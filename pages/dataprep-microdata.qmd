...
### Prepare Unit-Level Predictors

Unit-level models need predictor variables in the survey data that can also be represented in the population data. For the tutorial estimands, we keep five categorical predictors that are plausible for commute distance, bus satisfaction, and walking frequency.

| Predictor | Why it may be useful |
|---|---|
| `age_group` | Travel behaviour varies strongly over the life course. |
| `sex` | Travel patterns and service experiences can differ by sex. |
| `car_access` | Car availability affects commuting, bus use, and walking. |
| `economic_status` | Employment status is directly linked to commuting and daily travel routines. |
| `qualification_level` | Education can proxy socio-economic differences in work, travel, and service use. |
...
survey_unit_predictors <- nsw_raw |>
  transmute(
    ...
    car_access = case_when(
      CarUse == 1 ~ "car_access",
      CarUse == 2 ~ "no_car_access",
      TRUE ~ NA_character_
    ),
    economic_status = case_when(
      DvEcoStat3 == 1 ~ "econ_employed",
      DvEcoStat3 == 2 ~ "econ_unemployed",
      DvEcoStat3 == 3 ~ "econ_inactive",
      TRUE ~ NA_character_
    ),
    qualification_level = case_when(
      HighEd == 1 ~ "higher_education",
      HighEd == 2 ~ "no_higher_education",
      TRUE ~ NA_character_
    )
  )
...
| unit-level covariates | Categorical predictors used by unit-level models, when needed |
