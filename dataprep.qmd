...
### Defining The Estimand and Estimation Domain
...
In spatio-temporal SAE, this estimand is evaluated for every combination of area and time period. Each combination is an area-time cell: one observation we want to estimate. The *estimation domain* is therefore, all possible combination of areas and time periods.

For example, with the *target estimand*

{{< include assets/target-estimand.qmd >}}

The *estimation domain* is therefore, all possible combination of MSOAs-years.

### Preparing the Survey Indicator and Weight

The estimand is the proportion of the target population who cycle frequently. We therefore code the survey response as a binary indicator, `freq_cyclist`, because the average of a binary indicator gives the estimated prevalence. `freq_cyclist` is coded from `AtFrqBke`, the active-travel frequency question for cycling:

- `1`: `AtFrqBke` is `1` or `2`, meaning at least several times a week
- `0`: less frequent cycling or never cycling
- `NA`: missing, ineligible, or not asked

For each survey respondent, we also need a design weight, which adjusts the sample so that weighted estimates better represent the target population. Since the survey question was administered to all respondents during some survey periods and subsampled in others, the corresponding `weight` variable selection rule is as follows:
...
The survey design may include `stratum`, which records the sampling group each respondent belongs to. Including it helps the variance calculation reflect how the sample was drawn, if not randomised.

The survey indicator input has *one row per respondent-wave*:
...
| `strata` | Stratum (Local Authority) |
...
read_csv(file.path(base_path, "clean", "surveyind_freqcyc.csv"), show_col_types = FALSE) |>
...
read_csv(file.path(base_path, "clean", "surveyind_freqcyc.csv"), show_col_types = FALSE) |>
  select(unit_id, time_id, area_id, freq_cyclist, sex, age, caraccess) |>
  head()
```
...
read_csv(file.path(base_path, "clean", "areapred_freqcyc.csv"), show_col_types = FALSE) |>
  head()
```
...
The area-level predictors `age_adult_mean`, `female_adult_share`, and `caraccess_adult_share` are defined for the adult target population, so they can also be used as predictor summaries to aggregate unit-level model predictions to target small areas.
