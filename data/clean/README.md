...
- `survey_microdata_msoayear.csv`: one row per respondent-wave. Includes harmonised survey fields, synthetic MSOA assignment, cycling indicator, weights, and respondent predictors.
...
- `areapred_freqbike.csv`: one row per MSOA with raw and z-scored area predictors for frequent cycling models, including Census 2021 mean age among usual residents aged 16+ and proportion female from Nomis bulk tables.
...
- `scripts/build_clean_survey_microdata.py`: builds `survey_microdata_msoayear.csv`, `surveyind_freqbike.csv`, and `domain_msoayear.csv`.
- `scripts/build_cycling_area_predictors.py`: builds `areapred_freqbike.csv`.
- `scripts/build_cycling_poststrat_shallow3y.py`: builds `poststrat_freqbike_age_sex_212325.csv`.
- `scripts/build_cycling_poststrat_rich1y.py`: builds `poststrat_freqbike_age_sex_car_21.csv`.
