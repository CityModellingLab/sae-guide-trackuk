---
title: "Bayesian Spatiotemporal Small-area Estimation: A Practical Guide"
subtitle: "From sparse survey data to more reliable local estimates for urban researchers"
---

![](assets/banner.png){width="100%"}

## Small-area Estimation in Urban Analytics
...
![Bayesian inference combines survey evidence, auxiliary predictors, relationships between neighbouring areas, temporal dependence and uncertainty from sparse domains within a joint probability model.](assets/bayesiansae.png){width="100%"}

The workbooks use `R-INLA`, the R interface to [*Integrated Nested Laplace Approximation (INLA)*](https://www.r-inla.org/). This method provides deterministic approximate Bayesian inference for latent Gaussian models and is faster than Markov chain Monte Carlo (MCMC) methods for complex spatiotemporal small area estimation models (Rue et al, 2009).

## Navigating the guide

SAE methods commonly used with urban survey and auxiliary datasets are covered. Method selection depends on the target outcome, direct survey evidence, and auxiliary modelling and population data, each with trade-offs in terms of data requirements and the extent of population adjustments to fully or partially address sampling biases.
...
The data used was derived based on the [**National Survey for Wales**](https://www.gov.wales/national-survey-wales). The maps and estimates should not be interpreted as empirical findings about specific places in Wales. Auxiliary predictors, boundaries, and poststratification tables are derived from the *Office for National Statistics (ONS)*, including ONS UK Census 2021. The synthetic dataset construction and preprocessing steps are described in the [estimation setup](dataprep.qmd).
